...
function handleRequest(req, res) {
  ...
  // if request is type POST and on /exec|config.transport.path url then parse the data and handle it
  if (req.url.indexOf(config.transport.path) !== -1 && req.method.toLowerCase() === config.transport.method.toLowerCase() ) {
    return self.app.exec(req.body, function(err, out) {
      ...
      if (err) {
        ...
        return res.end(JSON.stringify(err, ["message", "arguments", "type", "name"]));
      }
  ...
  message = JSON.stringify({ message: 'To call an action use ' + self.config.transport.path });
